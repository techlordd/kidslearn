...
/* British first: a phonics app should sound consistent, and a child's
   accent shouldn't depend on which device they're using. A voice tagged
   en-GB wins even if we don't recognise its name; the named lists just
   pick the nicest-sounding one when there's a choice, and American voices
   are a last resort for devices with no British voice installed at all. */
function pickVoice(voices) {
  const english = voices.filter((v) => v.lang && v.lang.toLowerCase().startsWith('en'));
  const british = english.filter((v) => v.lang.toLowerCase() === 'en-gb');
  const likedBritish = ['google uk english female', 'google uk english male', 'daniel', 'arthur', 'kate', 'serena', 'hazel'];
  const likedAny = ['samantha', 'karen', 'moira', 'google us english', 'zira'];
  return (
    british.find((v) => likedBritish.some((n) => v.name.toLowerCase().includes(n))) ||
    british[0] ||
    english.find((v) => likedBritish.some((n) => v.name.toLowerCase().includes(n))) ||
    english.find((v) => likedAny.some((n) => v.name.toLowerCase().includes(n))) ||
    english[0] ||
    voices[0] ||
    null
  );
