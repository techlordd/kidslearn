...
import { MATHS_CORE_TOPICS, MATHS_TRAILS } from './maths';
import { rng, shuffle, pick } from './rng';

/* ── Grades ──────────────────────────────────────────────────────────────── */

// Content only covers the earliest two so far (phonics, plus the new maths
// starter pack) — the rest are here so a grown-up can already place an older
// child correctly, ready for lessons as they get written.
export const GRADES = [
  { id: 'reception', name: 'Reception', tagline: 'Ages 4–5' },
  { id: 'year1', name: 'Year 1', tagline: 'Ages 5–6' },
  { id: 'year2', name: 'Year 2', tagline: 'Ages 6–7' },
  { id: 'year3', name: 'Year 3', tagline: 'Ages 7–8' },
  { id: 'year4', name: 'Year 4', tagline: 'Ages 8–9' },
];
...
// Every trail (phonics' plus maths') that a core topic can belong to.
const ALL_TRAILS = [...TRAILS, ...MATHS_TRAILS];

export const CORE_TOPICS = [...SOUNDS.map(buildTopic), ...MATHS_CORE_TOPICS];
...
export function trailsForSubject(subjectId, customTopics = []) {
  ...
  const extras = {};
  topics
    .filter((t) => !ALL_TRAILS.some((tr) => tr.id === t.trail))
    .forEach((t) => {
      const key = t.trail;
...
// "Next" is relative to whatever subject the topic itself belongs to, so a
// maths topic keeps offering more maths next, not the phonics map.
export function nextTopicAfter(topicId, customTopics = []) {
  const topic = getTopic(topicId, customTopics);
  if (!topic) return null;
  const list = orderedTopics(topic.subject, customTopics);
